...
import torch
from .singlestream import SingleStream, SingleStreamResNet101
...
class ChangeDetectionModelResNet101(ChangeDetectionModel):
    def __init__(self, configuration):
        super().__init__(configuration)
        self.netchangedetection = SingleStreamResNet101(4, True, configuration["rgb"], configuration["depth"])
        self.model = self.netchangedetection
        print("Number total params ", sum(p.numel() for p in self.netchangedetection.parameters() if p.requires_grad))
        self.netchangedetection = self.netchangedetection.cuda()
