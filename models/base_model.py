...
def get_scheduler(optimizer, configuration, last_epoch=-1):
    ...
    if configuration['lr_policy'] == 'step':
        scheduler = lr_scheduler.StepLR(optimizer, step_size=configuration['lr_decay_iters'], gamma=configuration['decay'], last_epoch=last_epoch)
    elif configuration['lr_policy'] == 'cosine':
        scheduler = lr_scheduler.CosineAnnealingLR(optimizer, configuration['max_epochs']*configuration['batch_size'])
...
class BaseModel(ABC):
    ...
    def __init__(self, configuration):
        ...
        self.is_train = True if configuration["mode"] == "train" else False
        self.use_cuda = torch.cuda.is_available()
        self.device = torch.device('cuda:0') if self.use_cuda else torch.device('cpu')
        torch.backends.cudnn.benchmark = True
        self.save_dir = configuration['save_path']
        self.weights_dir = configuration['weights_path']
        self.network_names = []
        self.loss_names = []
        self.optimizers = []
        self.visual_names = []
    ...
    def set_input(self, inputs):
        ...
        """
        self.input = inputs[0].cuda()
        self.label = inputs[1].cuda()
    ...
    def setup(self):
        ...
        if self.configuration['load_weights'] > 0:
            last_checkpoint = self.configuration['load_weights']
        else:
            last_checkpoint = -1
    ...
    def export(self):
        ...
        for name in self.network_names:
            if isinstance(name, str):
                net = getattr(self, 'net' + name)
                export_path = os.path.join(self.configuration['save_path'], 'exported_net_{}.pth'.format(name))
